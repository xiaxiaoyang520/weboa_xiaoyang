package com.hbpu.weboa.web.consts;
...
public class GeetestLib {
	...
	private int registerChallenge() {
		try {
			...
			Gson gson = new Gson();
			JsonObject jsonObject = gson.fromJson(result_str, JsonObject.class);
			...
			LOG.error("exception:register api", e);
		}
		return 0;
	}
	...
	public int enhencedValidateRequest(String challenge, String validate, String seccode) {
		...
		try {
			...
			LOG.error("checkResultByPrivate error", e);
		}
		...
		try {
			if (StringUtils.hasText(response) && response.startsWith("{")) {
				...
				return_seccode = jsonObject.get("seccode").getAsString();
